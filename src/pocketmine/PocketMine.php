...
namespace pocketmine {
	...
	if(version_compare("7.2.0", PHP_VERSION) > 0){
		echo "[CRITICAL] You must use PHP >= 7.2.0" . PHP_EOL;
	...
	if(extension_loaded("pocketmine")){
		if(version_compare(phpversion("pocketmine"), "0.0.1") < 0){
			$logger->critical("You have the native Apollo extension, but your version is lower than 0.0.1.");
			++$errors;
		}elseif(version_compare(phpversion("pocketmine"), "0.0.4") > 0){
			$logger->critical("You have the native Apollo extension, but your version is higher than 0.0.4.");
			++$errors;
		}
	}

	if(extension_loaded("xdebug")){
		$logger->warning("You are running Apollo with Xdebug enabled. This has a major impact on performance.");
	}
